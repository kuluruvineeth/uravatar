...
export const ProjectContext = createContext<{
  ...
  setPromptImageUrl: (promptImageUrl: string | undefined) => void;
}>(null!);
...
export const ProjectProvider = ({
  ...
  const [skip, setSkip] = useState(SHOTS_PER_PAGE);
  ...
  const { isLoading: isLoadingMore, refetch } = useQuery(
    `shots-${SHOTS_PER_PAGE}-${skip}`,
    () =>
      axios.get<{ shots: Shot[]; shotsCount: number }>(
        `/api/projects/${project.id}/shots?take=${SHOTS_PER_PAGE}&skip=${skip}`
      ),
    {
      enabled: false,
      onSuccess: (response) => {
